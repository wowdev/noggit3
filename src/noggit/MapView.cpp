    ...
    for (auto& selection : currentSelection)
    {
      ...
      if (canMoveObj && (keyx != 0 || keyy != 0 || keyz != 0 || keyr != 0 || keys != 0))
      {
        ...
        if (selection.which() == eEntry_WMO)
        {
          ...
          _world->updateTilesWMO(wmo, model_update::remove);
          wmo->pos.x += keyx * moveratio;
          wmo->pos.y += keyy * moveratio;
          wmo->pos.z += keyz * moveratio;
          wmo->dir.y += keyr * moveratio * 5;
          wmo->recalcExtents();
          _world->updateTilesWMO(wmo, model_update::add);
        ...
        if (selection.which() == eEntry_Model)
        {
          ...
          model->pos.x += keyx * moveratio;
          model->pos.y += keyy * moveratio;
          model->pos.z += keyz * moveratio;
          model->dir.y += keyr * moveratio * 5;
          model->scale += keys * moveratio / 50;
          model->recalcExtents();
