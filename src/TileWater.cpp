		...
		if(Header[i][j].nLayers > 0){
		  for(int k=0; k < Header[i][j].nLayers; ++k){
			...
			if(Info[i][j][k].ofsInfoMask > 0){
				int numBytes = (Info[i][j][k].width * Info[i][j][k].height) / 8;
...
void TileWater::saveToFile(sExtendableArray &lADTFile, int &lMHDR_Position, int &lCurrentPosition){
	...
	for(int i=0; i < 16; ++i) {
      for(int j=0; j < 16; ++j) {
		...
		lADTFile.GetPointer<MH2O_Header>(ofsW + (16 * i + j) * sizeof(MH2O_Header))->ofsInformation = lCurrentPosition - ofsW; //setting offset to this info at header
		lADTFile.GetPointer<MH2O_Header>(ofsW + (16 * i + j) * sizeof(MH2O_Header))->nLayers = 1;
	...
	for(int i=0; i < 16; ++i) {
      for(int j=0; j < 16; ++j) {
		...
		if(used.Render[i][j]){
			...
			for(int h=0 ; h < 8; ++h) {
					for(int w=0; w < 8; ++w) {
						Render[i][j][0].mRender[w][h]? (wRender[w] |= (1<<h)) : (wRender[w] &=~(1<<h));
						Render[i][j][0].fRender[w][h]? (fRender[w] |= (1<<h)) : (fRender[w] &=~(1<<h));
					}
				}
		...
		if(used.Mask[i][j]){
			lADTFile.Insert(lCurrentPosition, (Info[i][j][0].width * Info[i][j][0].height) / 8, reinterpret_cast<char*>(&Mask[i][j][0]));
			...
			waterSize += (Info[i][j][0].width * Info[i][j][0].height) / 8;
			lCurrentPosition +=  (Info[i][j][0].width * Info[i][j][0].height) / 8;
		}
		...
		if(used.TransparencyData[i][j]){	
			for (int w = Info[i][j][0].yOffset; w < Info[i][j][0].yOffset + Info[i][j][0].height + 1; ++w) {
				for(int h=Info[i][j][0].xOffset; h < Info[i][j][0].xOffset + Info[i][j][0].width + 1; ++h) {
					lADTFile.Insert(lCurrentPosition, sizeof(char), reinterpret_cast<char*>(&HeightData[i][j][0].mTransparency[w][h]));
					lCurrentPosition+=sizeof(char);
					waterSize += sizeof(char);
				}
			}
		}
	  }
